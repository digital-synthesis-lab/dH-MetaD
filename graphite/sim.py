...
def get_args(argv):
    ...
    available_args = [
        ...
        ("temperature", "T", int, 1000, "Temperature of the simulation"),
        ("pressure", "p", int, 1, "Pressure of the simulation"),
        ("height", "y", float, 0.1, "Height of gaussian bias"),
        ("width", "w", float, 0.5, "Width of gaussian bias"),
...
refer = read(f'results_unbiased/graphite_{args.temperature}K_{args.pressure}bar.traj', index=':', format='lammps-dump-text')[50:]
...
def main(argv):
    ...
    cache = initialize_cache(f'structure/graphite_{temperature}K_{pressure}bar.lmp')
